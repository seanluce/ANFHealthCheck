...
$enablePoolCapacityRemediationDryRun = $false #false will resize capacity pools down to desired headroom
...
$minPoolSizeGiB = 4096 # use this to set the minimum pool size in GiB
...
$credentials = Get-AutomationPSCredential -Name "SeanLuceSP"
Connect-AzAccount -ServicePrincipal -Credential $credentials -Tenant "72f988bf-86f1-41af-91ab-2d7cd011db47"

function Send-Email() {
    #####
    ## Send finalResult as email
    #####
    $Username ="sean@seanluce.com" # Your user name - found in SendGrid portal
    $Password = ConvertTo-SecureString "2ry6ps7et33l58k2" -AsPlainText -Force # SendGrid password
    $credential = New-Object System.Management.Automation.PSCredential $Username, $Password
    $SMTPServer = "smtp.fastmail.com"
    $EmailFrom = "sean@seanluce.com" # Can be anything - aaa@xyz.com
    #$EmailTo = "ng-anf-pm-lab@netapp.com" # Valid recepient email address
    #$EmailTo = @('luces@netapp.com', 'geert.vanteylingen@netapp.com', 'arnt.degier@netapp.com')
    $EmailTo = @('luces@netapp.com')
	$Body = $finalResult
...
function Get-ANFPoolDetails($poolAllocatedSizes) {
    ...
    foreach($pool in $capacityPools) {
        ...
        Write-Output $desiredHeadroomPercent #todo
